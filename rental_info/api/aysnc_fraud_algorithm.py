...
def check_address(state, city, street, price, list_id=None):
    crawler_collection = CRAWLERDBCLIENT.scrapy.scrapyItems
    addressStr = street.split()
    street = addressStr[:len(addressStr) - 1]
    street = ' '.join(street) + ' ' + addressStr[len(addressStr) - 1][0]

    query_result = crawler_collection.find({'address.state': {'$regex' : state,'$options' : 'i'},
